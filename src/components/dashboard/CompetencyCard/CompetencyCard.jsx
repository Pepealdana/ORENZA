...
function CompetencyCard({ name, progress, completedCount = 0, totalActivities = 0, to, icon }) {
  return (
    <article className={styles.card}>
      ...
      <ProgressBar value={progress} showValue={false} label={`Progreso en ${name}`} />
      <p className={styles.experienceCount}>
        {completedCount} de {totalActivities} {totalActivities === 1 ? 'experiencia explorada' : 'experiencias exploradas'}
      </p>
      {to && <Link to={to} className={styles.detailLink}>Explorar <ArrowRight size={16} /></Link>}
    </article>
  );
}
